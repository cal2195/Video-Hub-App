...
export function watchInputFolder(
  ...
  finalArray.forEach((element) => {
    if (!dirs[element.partialPath]) {
      dirs[element.partialPath] = element.partialPath;
    }
  });

  let data = {}; //used to correlate two events

  const watchCallback = (event) => {
      let mask = event.mask;
